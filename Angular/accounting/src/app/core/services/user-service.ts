...
import { CompanyModel, ledger, userAccessTokenDecode, VoucherType } from '../../datamodels/datamodels';
...
export class UserService {
  ...
  selectedCompany_id =signal<number>(0)
  selectedCompanyLedgerList = signal<ledger[]>([]);
  // voucherTypeList = signal<VoucherType[]>([]);
   ...
   getSelectedCompanyLedger() {
      ...
      if (this.selectedCompany().id != undefined) {
        this.http.get<ledger[]>('ledger/', { company: this.selectedCompany().id}).subscribe((value) => {
          ...
          // console.log(this.selectedCompanyLedgerList());
