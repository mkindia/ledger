...
import { AccountTransaction, entry, ledger, transaction } from '../../datamodels/datamodels';
...
@Component({
  ...
  imports: [MatButtonModule, TitleCasePipe,
    ...
    ReactiveFormsModule, MatButtonModule, MatCardModule, MatGridListModule, MatIcon, TitleCaseDirective, NgClass],
...
export class Transation implements AfterViewInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    layout.appLayOut.set({
      XSmall: { colspan_1: 4, colspan_2: 8, colspan_3: 4, colspan_4: 8, colspan_5: 0, colspan_6: 0, rowspan_1: 1 },
      Small: { colspan_1: 4, colspan_2: 8, colspan_3: 4, colspan_4: 8, colspan_5: 0, colspan_6: 0, rowspan_1: 1 },
  ...
  SampelEditData: [] = [];
  ngOnInit() {
    this.voucher_type = this.router.url.substring(1);
  ...
  ngAfterViewInit() {
    setTimeout(() => {
      ...
      this.editLedger();
      // console.log(this.userService.getSelectedCompanyLedger());
  ...
  editLedger() {
    ...
    if (this.router.url.substring(1) == `transaction/${sp}`) {
      this.http.get<transaction>(`${this.router.url.substring(1)}/`).subscribe((value1: transaction) => {
        ...
        this.voucher_type = value1.voucher_type.toUpperCase();
        this.entries.clear();
  ...
  filter(index: number, value: string) {
    ...
    if (this.voucher_type.toLowerCase() == 'payment' && entry_type != 'debit') {
      this.filteredLedger = this.userService.selectedCompanyLedgerList().filter(o => o.name.toLowerCase().includes(filterValue) && o.group == 'bank accounts' || o.name.toLowerCase().includes(filterValue) && o.group == 'cash-in-hand');
    }
    else if (this.voucher_type.toLowerCase() == 'payment' && entry_type != 'credit') {
      this.filteredLedger = this.userService.selectedCompanyLedgerList().filter(o => o.name.toLowerCase().includes(filterValue) && o.group == 'sundry debtors' || o.name.toLowerCase().includes(filterValue) && o.group == 'sundry creditors');
    }
      ...
      this.filteredLedger = this.userService.selectedCompanyLedgerList().filter(o => o.name.toLowerCase().includes(filterValue) && o.group == 'sales accounts');
    }
      ...
      this.filteredLedger = this.userService.selectedCompanyLedgerList().filter(o => o.name.toLowerCase().includes(filterValue) && o.group == 'purchase accounts');
    }
  ...
  submit() {
    ...
    if (this.entryForm.valid) {
      ...
      this.entryForm.value.entries.forEach((element: any) => {
        entries.push({ ledger: element.ledger.id, entry_type: element.entry_type, amount: element.amount, narration: element.narration })
      });

      let data1: transaction = {
        "voucher_no": "pay001",
        "voucher_type": this.router.url.substring(1),
